...
module Beaker
  describe Answers do
    ...
    it 'generates 3.4 answers for 3.4 hosts' do
      @ver = '3.4'
      Beaker::Answers::Version34.should_receive( :answers ).with( hosts, {}).once
      subject.answers( @ver, hosts, {} )
    end

    it 'generates 3.2 answers for 3.3 hosts' do
      @ver = '3.3'
      Beaker::Answers::Version32.should_receive( :answers ).with( hosts, {}).once
      subject.answers( @ver, hosts, {} )
    end

    it 'generates 3.2 answers for 3.2 hosts' do
      @ver = '3.2'
      Beaker::Answers::Version32.should_receive( :answers ).with( hosts, options ).once
      subject.answers( @ver, hosts, options )
    end

    it 'generates 3.0 answers for 3.1 hosts' do
      @ver = '3.1'
      Beaker::Answers::Version30.should_receive( :answers ).with( hosts, options ).once
      subject.answers( @ver, hosts, options )
    end
    ...
    it 'generates 2.8 answers for 2.8 hosts' do
      @ver = '2.8'
      Beaker::Answers::Version28.should_receive( :answers ).with( hosts, options ).once
      subject.answers( @ver, hosts, options )
    end

    it 'generates 2.0 answers for 2.0 hosts' do
      @ver = '2.0'
      Beaker::Answers::Version20.should_receive( :answers ).with( hosts, options ).once
      subject.answers( @ver, hosts, options )
    end

    it 'raises an error for an unknown version' do
      @ver = 'x.x'
      expect{ subject.answers( @ver, hosts, options ) }.to raise_error( NotImplementedError )
    end
  end
  ...
  module Answers
    describe Version34 do
      ...
      it 'should add answers to the host objects' do
        @ver = '3.4'
        answers = subject.answers( hosts, options )
        hosts.each do |host|
    ...
    describe Version32 do
      ...
      it 'should add q_puppetmaster_certname to the dashboard answers' do
        @ver = '3.2'
        expect( subject.answers( hosts, options )['vm2']).to include :q_puppetmaster_certname
      end

      it 'should add q_upgrade_with_unknown_disk_space to the dashboard on upgrade' do
        @ver = '3.2'
        expect( subject.answers( hosts, options.merge( {:type => :upgrade} ) )['vm2']).to include :q_upgrade_with_unknown_disk_space
      end
    ...
    describe Version28 do
      ...
      it 'sets correct answers for an agent' do
        @ver = '2.8'
        expect( subject.answers( hosts, options )['vm3'] ).to be === { :q_install=>"y", :q_puppetagent_install=>"y", :q_puppet_cloud_install=>"y", :q_puppet_symlinks_install=>"y", :q_vendor_packages_install=>"y", :q_puppetagent_certname=>hosts[2], :q_puppetagent_server=>hosts[0], :q_puppetmaster_install=>"n", :q_puppet_enterpriseconsole_install=>"n" }
      end
